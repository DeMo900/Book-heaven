...
useEffect(()=>{
    if(genre === "All"){
setbooks(AllBooks.filter((book)=>( book.title.toLowerCase().includes(search.toLowerCase()))))
    }else{
      setbooks(AllBooks.filter((book)=>(book.genre === genre && book.title.toLowerCase().includes(search.toLowerCase()))))
    }
},[search])
    useEffect(()=>{
        console.log(genre)
  ...
  //  setbooks(AllBooks)
        console.log(books.filter((book)=>( book.genre === genre)))
    },[genre])
...
const handleBookLikeClick = async(title:string,newLiked:boolean)=>{
   ...
   await fetch (`http://localhost:9000/books?stared=${newLiked}&title=${encodeURIComponent(title)}`,{
    ...
    return (
        <div>
          <Navbar isSearchHidden={false} onChange={(e)=>setSearch(e.target.value)}/>
...
<p className="text-[#486459] font-[Inter] text-xl leading-6 md:max-w-2xl md:leading-10 md:text-4xl">
  ...
  <div className="flex flex-wrap gap-2 items-center  mt-18 md:mt-20 p-2 w-full ">
    ...
    onClick={()=>{genre === g ? setGenre("All") :setGenre(g)}}
    >{g}</div>
    ))}
  </div>
